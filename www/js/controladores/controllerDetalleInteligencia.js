var adnApp = angular.module('DETALLE_INTELIGENCIA', []);

adnApp.controller('controllerDetalleInteligencia',function($scope,$rootScope,$http,$ionicLoading,$ionicPopup,$state) {
    $scope.data = [];
  ...
  $scope.hide = function(){
	...
	$scope.show();
        ...
        $scope.parametros = $rootScope.idDimension+"@#"+$rootScope.idRubro+"@#";
        $http.post($scope.url, {"usuario":$rootScope.idUsuario,"password":$rootScope.password,"empresa":$rootScope.idEmpresa,"sql":$scope.sql,"parametros":$scope.parametros,"hash":$scope.hash}).
          ...
          $scope.respuesta = $scope.resultado.return.toString();
                      ...
                      var ArrayItems = [];    $rootScope.ArrayDetalleDimension = []; 
                      ...
                      for (var i = 0; i < ArrayItems[0].length; i++) {
                       ...
                       $rootScope.ArrayDetalleDimension.push(itemArray);
                      }console.log($rootScope.ArrayDetalleDimension);    
        });
})